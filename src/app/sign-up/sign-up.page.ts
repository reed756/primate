...
export class SignUpPage implements OnInit {
  ...
  ngOnInit() {
    this.nationalities.forEach(element => {
      this.newNationalities.push(element.Nationality);
    }
    );
  }
  ...
  handleInput(ev: any) {
    if (ev.target.value.length === 0) {
      ...
      this.results = this.newNationalities.filter((d: string) => d.toLowerCase().indexOf(query) > -1);
    }
  }

  setNationality(nationality: string) {
    this.signUpForm.patchValue({ nationality }); // Update the form's nationality control
    this.searchBar.value = nationality;
    this.results = [];
  }
